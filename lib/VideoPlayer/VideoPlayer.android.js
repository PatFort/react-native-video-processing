...
export class VideoPlayer extends Component {
  ...
  getVideoInfo() {
    ...
    UIManager.dispatchViewManagerCommand(findNodeHandle(this), ProcessingUI.Commands.getInfo, [])
  ...
  trim(options) {
    if (typeof options === "string") {
      console.warn(
        "There is no need to pass source for trimming, this is deprecated and will be removed on the next version"
      )
      return Promise.reject()
    }
    const { startTime, endTime } = options
    UIManager.dispatchViewManagerCommand(findNodeHandle(this), ProcessingUI.Commands.trim, [
      startTime,
      endTime
    ])
    return new Promise(resolve => {
  ...
  compress(_options = {}) {
    ...
    UIManager.dispatchViewManagerCommand(findNodeHandle(this), ProcessingUI.Commands.compress, [
      options
    ])
    return new Promise(resolve => {
  ...
  render() {
    ...
    if (__DEV__) {
      const isCompatible = Object.values(VideoPlayer.Constants.resizeMode).includes(resizeMode)
